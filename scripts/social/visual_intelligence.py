...
def build_art_direction(
    *,
    ...
    return ArtDirection(
        ...
        secondary_subjects=[],
        action="",
        environment="specific, credible power-use setting with layered practical detail",
        composition="dense high-impact product poster: oversized product cutout crossing the center line, stacked headline interacting with the product silhouette, diagonal information bands, compact proof rail, and intentional visual weight in every canvas zone",
        focal_point=primary_subject,
        depth="deep layered foreground, product, and environmental backplate",
        camera_angle="dynamic low three-quarter product hero angle",
        ...
        texture="physical grain, material texture, diagonal light streaks, and restrained utility-pattern overlays",
        mood=", ".join(brand.get("brand_mood", {}).get("primary_words", [])) or "charged and assured",
        style="layered high-end performance advertising with compact editorial proof modules",
        ...
        brand_connection="small wordmark in a deliberate corner lockup; no fake product recreation",
        text_safe_area=f"padding {plat.get('safe_area_padding_px', 96)}px; logo zone {plat.get('logo_zone_px', 160)}px",
        must_include=["oversized real product hero", "stacked type hierarchy", "dense proof rail", "full-canvas visual weight"],
        must_avoid=must_avoid,
