...
function contentTitle(item) { const packageValue = item.package || {}; return packageValue.title || packageValue.product_name || packageValue.objective || packageValue.post_id || item.content_id || 'Untitled content'; }
function contentCaption(item) { const packageValue = item.package || {}; const posts = packageValue.platform_posts || packageValue.platform_presentations || {}; const first = Object.values(posts)[0]; return String(first?.final_caption || first || packageValue.master_copy || packageValue.fb_caption || packageValue.ig_caption || packageValue.li_text || ''); }
function filteredContent(items) { const filters = state.contentFilters; return (items || []).filter((item) => { const searchable = [contentTitle(item), contentCaption(item), item.outbox_id, item.content_id, item.content_date, item.content_type, ...(item.transactions || []).map((transaction) => transaction.external_id)].join(' ').toLowerCase(); return (!filters.query || searchable.includes(filters.query.toLowerCase())) && (!filters.platform || (item.platforms || []).includes(filters.platform) || (item.transactions || []).some((transaction) => transaction.platform === filters.platform)) && (!filters.status || item.status === filters.status) && (!filters.content_type || item.content_type === filters.content_type); }); }
function contentRecord(item, published = false) { const caption = contentCaption(item); const receipts = item.confirmed_publications || []; const attention = item.last_error || (item.rejection_reasons || []).join(', '); const actions = published ? `<button type="button" data-content-action="republish" data-outbox-id="${esc(item.outbox_id)}">New Gemini version</button>` : `<button type="button" data-content-action="edit" data-outbox-id="${esc(item.outbox_id)}">Edit copy & type</button><button type="button" data-content-action="reschedule" data-outbox-id="${esc(item.outbox_id)}">Reschedule</button><button type="button" data-content-action="remake" data-outbox-id="${esc(item.outbox_id)}">Remake with GPT</button><button type="button" data-content-action="regenerate" data-outbox-id="${esc(item.outbox_id)}">Regenerate with Gemini</button><button type="button" data-content-action="hold" data-outbox-id="${esc(item.outbox_id)}">Hold</button><button type="button" class="danger" data-content-action="publish" data-outbox-id="${esc(item.outbox_id)}">Publish now</button>`; return `<article class="content-record"><div class="content-record-date"><b>${esc(new Date(item.scheduled_at).toLocaleDateString([], { month: 'short', day: 'numeric' }))}</b><span>${esc(new Date(item.scheduled_at).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }))}</span></div><div class="content-record-main"><div class="content-record-meta">${pill(item.status)}<span>${esc(humanize(item.content_type))}</span><span>${esc(humanize(item.slot))}</span></div><h3>${esc(contentTitle(item))}</h3><p>${esc(caption || 'No final caption stored.')}</p><div class="content-platforms">${(item.platforms || []).map((platform) => `<span>${esc(humanize(platform))}</span>`).join('')}</div>${attention ? `<div class="inline-alert">${esc(attention)}</div>` : ''}<div class="content-record-actions">${actions}</div></div><div class="content-record-evidence">${published ? receipts.map((receipt) => `<span><b>${esc(humanize(receipt.platform))}</b><code>${esc(receipt.external_id || 'missing external ID')}</code></span>`).join('') : `<span><b>Prepare by</b>${esc(dateTime(item.prepare_by))}</span><span>${item.operationally_visible ? pill('Visible now', 'READY') : pill('Scheduled', 'PENDING')}</span>`}<button type="button" class="ghost" data-content-inspect="${esc(item.outbox_id)}">Inspect</button></div></article>`; }
function populateContentFilters() { const facets = state.contentOperations?.facets || {}; document.querySelectorAll('[data-content-filters]').forEach((container) => { for (const field of ['platform', 'status', 'content_type']) { const select = container.querySelector(`[data-content-filter="${field}"]`); const values = facets[field === 'content_type' ? 'content_types' : `${field}s`] || []; const label = field === 'content_type' ? 'content types' : field === 'status' ? 'statuses' : 'platforms'; select.innerHTML = `<option value="">All ${label}</option>${values.map((value) => `<option value="${esc(value)}">${esc(humanize(value))}</option>`).join('')}`; select.value = state.contentFilters[field]; } container.querySelector('[data-content-filter="query"]').value = state.contentFilters.query; }); }
function renderContentOperations() { const workspace = state.contentOperations; if (!workspace) return; const upcoming = filteredContent(workspace.upcoming); const published = filteredContent(workspace.published); $('#content-operations-metrics').innerHTML = [['Upcoming', workspace.summary.upcoming], ['Published', workspace.summary.published], ['Needs attention', workspace.summary.needs_attention], ['Media assets', workspace.summary.media], ['Preparation', `${workspace.windows.preparation_hours}h`]].map(([label, value]) => `<div class="metric panel"><b>${esc(value)}</b><span>${esc(label)}</span></div>`).join(''); $('#upcoming-window').textContent = `${workspace.windows.upcoming_start} to ${workspace.windows.upcoming_end}`; $('#published-window').textContent = `${workspace.windows.history_start} to ${workspace.windows.history_end}`; $('#upcoming-content-list').innerHTML = upcoming.length ? upcoming.map((item) => contentRecord(item)).join('') : empty('No upcoming content matches these filters'); $('#published-content-list').innerHTML = published.length ? published.map((item) => contentRecord(item, true)).join('') : empty('No confirmed publications match these filters'); $('#media-count').textContent = `${workspace.media.length} assets`; $('#media-library-grid').innerHTML = workspace.media.length ? workspace.media.map((asset) => { const url = mediaUrl(asset.source); return `<article class="media-library-item">${url ? `<a href="${esc(url)}" target="_blank" rel="noopener"><img src="${esc(url)}" alt="${esc(humanize(asset.role))}" loading="lazy"></a>` : ''}<div><div>${pill(asset.status)}<span>${esc(humanize(asset.role))}</span></div><code>${esc(asset.sha256 || asset.identity)}</code><small>${asset.outbox_ids.length} linked post${asset.outbox_ids.length === 1 ? '' : 's'}</small>${url ? `<a class="media-download" href="${esc(url)}" download>Download</a>` : ''}</div></article>`; }).join('') : empty('No stored media in this window'); populateContentFilters(); }
function inspectContent(outboxId) { const records = [...(state.contentOperations?.upcoming || []), ...(state.contentOperations?.published || [])]; const item = records.find((record) => record.outbox_id === outboxId); if (!item) return; const packageValue = item.package || {}; const posts = packageValue.platform_posts || packageValue.platform_presentations || {}; const readinessText = JSON.stringify((item.readiness || {}), null, 2); $('#content-inspector-body').innerHTML = `<header><div><p class="kicker">${esc(item.outbox_id)}</p><h2>${esc(contentTitle(item))}</h2></div>${pill(item.status)}</header><dl><dt>Scheduled</dt><dd>${esc(dateTime(item.scheduled_at))}</dd><dt>Prepare by</dt><dd>${esc(dateTime(item.prepare_by))}</dd><dt>Type</dt><dd>${esc(humanize(item.content_type))}</dd><dt>Readiness</dt><dd><pre>${esc(readinessText)}</pre></dd></dl><div class="inspector-captions">${Object.entries(posts).map(([platform, value]) => `<section><b>${esc(humanize(platform))}</b><p>${esc(value?.final_caption || value || '')}</p></section>`).join('') || '<p>No platform captions stored.</p>'}</div><div class="inspector-transactions"><h3>Platform evidence</h3>${(item.transactions || []).map((transaction) => `<section>${pill(transaction.state)}<b>${esc(humanize(transaction.platform))}</b><code>${esc(transaction.external_id || transaction.request_key)}</code><small>${esc(dateTime(transaction.updated_at))}${transaction.last_error ? ` · ${esc(transaction.last_error)}` : ''}</small></section>`).join('') || '<p>No dispatch transaction exists.</p>'}</div>`; $('#content-inspector').showModal(); }
async function loadContentOperations() { state.contentOperations = await api('/api/os/content-operations', { method: 'POST', body: JSON.stringify({ upcoming_days: 30, history_days: 30 }) }); renderContentOperations(); }
async function executeContentAction(action, outboxId) {
  const item = [...(state.contentOperations?.upcoming || []), ...(state.contentOperations?.published || [])].find((record) => record.outbox_id === outboxId);
  if (!item) throw new Error('Content record is no longer available.');
  let capability = '';
  let argumentsValue = { outbox_id: outboxId };
  if (action === 'edit') { const title = window.prompt('Post title', contentTitle(item)); if (title === null) return; const postType = window.prompt('Post type: current_event, product_education, statement, humor, framework, micro_story, explainer, drill, or myth', item.package?.post_type || item.content_type || 'statement'); if (!postType) return; const caption = window.prompt('Primary platform caption', contentCaption(item)); if (caption === null) return; const platformPosts = Object.fromEntries((item.platforms || []).map((platform) => [platform, { final_caption: caption }])); capability = 'publication.edit_ready'; argumentsValue = { ...argumentsValue, content_date: item.content_date, changes: { title, post_type: postType, platform_posts: platformPosts } }; }
  if (action === 'hold') { capability = 'publication.hold'; argumentsValue.reason = window.prompt('Reason for holding this publication', 'Owner review required') || ''; if (!argumentsValue.reason) return; }
  if (action === 'remake') { const instruction = window.prompt('Tell gpt-5.6-sol how to remake this post', 'Make the hook more direct while preserving verified facts and the approved visual.'); if (!instruction) return; capability = 'publication.master_remake'; argumentsValue = { ...argumentsValue, content_date: item.content_date, instruction }; }
  if (action === 'regenerate') { capability = 'publication.correct_ready'; argumentsValue = { ...argumentsValue, content_date: item.content_date, reason: 'Owner requested regeneration from Content Operations' }; }
  if (action === 'republish') { capability = 'publication.republish'; argumentsValue = { ...argumentsValue, content_date: item.content_date, reason: 'Owner requested a new Gemini version from Content Operations' }; }
  if (action === 'publish') { if (!window.confirm(`Publish ${contentTitle(item)} now to ${(item.platforms || []).map(humanize).join(', ')}? This requires owner approval and real provider receipts.`)) return; capability = 'publication.dispatch'; argumentsValue.publish_now = true; }
  if (action === 'reschedule') { const local = window.prompt('New local date and time', String(item.scheduled_at).slice(0, 16)); if (!local) return; const scheduled = new Date(local); if (Number.isNaN(scheduled.getTime())) throw new Error('Enter a valid date and time.'); capability = 'publication.reschedule'; argumentsValue = { ...argumentsValue, scheduled_at: scheduled.toISOString(), slot: item.slot }; }
  const result = await api('/api/os/execute', { method: 'POST', body: JSON.stringify({ capability, arguments: argumentsValue, operation_id: `content-${action}-${outboxId}-${Date.now()}` }) });
  await load();
  if (result.status === 'WAITING_APPROVAL') { activateView('command'); toast('Exact action is waiting for owner approval'); }
  else toast(`${humanize(action)} completed`);
}

function persistContentFilters() { const location = new URL(window.location.href); for (const [field, value] of Object.entries(state.contentFilters)) { const key = field === 'content_type' ? 'type' : field; if (value) location.searchParams.set(key, value); else location.searchParams.delete(key); } history.replaceState({}, '', location); }
...
async function load() { try { const [data] = await Promise.all([api('/api/os/state'), loadCreatives(), loadGenerationRequests(), loadMaster(), loadTikTokStatus(), loadSocialCalendar(), loadContentPlan(), loadContentOperations()]); state.data = data; state.conversationId = data.conversation?.id; syncConversation(data.conversation); render(data); $('#system-dot').className = 'dot ok'; $('#system-label').textContent = 'Master OS connected'; } catch (error) { $('#system-dot').className = 'dot bad'; $('#system-label').textContent = 'Connection blocked'; toast(error.message); } }
function activateView(view) {
  ...
  const location = new URL(window.location.href); location.searchParams.set('view', view); history.replaceState({}, '', location);
...
document.addEventListener('input', (event) => { const input = event.target.closest('[data-content-filter="query"]'); if (!input) return; state.contentFilters.query = input.value.trim(); persistContentFilters(); renderContentOperations(); });
document.addEventListener('change', (event) => { const input = event.target.closest('[data-content-filter]'); if (!input || input.dataset.contentFilter === 'query') return; state.contentFilters[input.dataset.contentFilter] = input.value; persistContentFilters(); renderContentOperations(); });
document.addEventListener('click', (event) => { const button = event.target.closest('[data-content-reset]'); if (!button) return; state.contentFilters = { query: '', platform: '', status: '', content_type: '' }; persistContentFilters(); renderContentOperations(); });
document.addEventListener('click', (event) => { const button = event.target.closest('[data-content-inspect]'); if (button) inspectContent(button.dataset.contentInspect); });
document.addEventListener('click', async (event) => { const button = event.target.closest('[data-content-action]'); if (!button) return; try { button.disabled = true; await executeContentAction(button.dataset.contentAction, button.dataset.outboxId); } catch (error) { button.disabled = false; toast(error.message); } });
...
const callbackResult = initialParams.get('tiktok');
for (const field of Object.keys(state.contentFilters)) state.contentFilters[field] = initialParams.get(field === 'content_type' ? 'type' : field) || '';
if (state.token) { showApp(); const requestedView = initialParams.get('view'); if (requestedView) activateView(requestedView); load().then(() => { if (callbackResult) { activateView('social'); toast(callbackResult === 'connected' ? 'TikTok connected' : 'TikTok authorization was not completed'); } }); } else showLogin();
