...
function planEntry(entry) {
  ...
  const quoteDelivery = entry.layout === 'single_frame_integrated_typography' ? `<dt>Quote delivery</dt><dd>${esc(entry.delivery_label)} · ${esc(entry.canvas_px.width)}×${esc(entry.canvas_px.height)} · exact text appears once</dd><dt>Exact company quote</dt><dd>${esc(entry.exact_visible_text[0])}</dd><dt>Company source</dt><dd>${esc(companySource.knowledge_id)} · ${esc(companySource.thought_id)} · verbatim</dd><dt>Source audience</dt><dd>${esc(humanize(companySource.audience))} · ${esc(humanize(companySource.pillar))}</dd>` : '';
