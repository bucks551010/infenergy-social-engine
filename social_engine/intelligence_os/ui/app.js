...
function approvalSummary(item) { const request = item.request || {}; const scope = request.permissions?.scope || {}; const range = scope.start_date && scope.end_date ? `${scope.start_date}–${scope.end_date}` : ''; return request.objective || request.name || request.rationale || [request.campaign || scope.campaign, range].filter(Boolean).join(' · ') || 'Owner authorization required'; }
